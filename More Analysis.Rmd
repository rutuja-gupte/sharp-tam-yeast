...
rep.df %>% ggplot() +
  ...
  # geom_point
  geom_errorbar(aes(x = group, ymin = lower/624598, ymax = upper/624598, color=bases), width=0.03) +
  geom_point(aes(x=group, y=fraction/624598, color=bases)) + 
  geom_smooth(aes(x=group, y=fraction/624598, color=bases), linetype="dotted", method="lm", se=FALSE) +
...
```{r}
model1 <- glm(SNM ~ length + reads + length*reads, data=df, family="poisson")
summary(model1)
```

```{r}
model2 <- glm(SNM ~ length + reads + length*reads, data=df, family="quasipoisson")
summary(model2)
```
...
```{r}
model3 <- glm.nb(SNM ~ length + reads + length*reads, data=df)
summary(model3)
```
...
```{r}
model4 <- glm(SNM ~ length:reads, data=df, family="quasipoisson")
summary(model4)
```
...
```{r}
model5 <- glm(SNM ~ length + reads + length:reads + protein, data=df, family="quasipoisson")
summary(model5)
```
...
df %>% ggplot() +
  geom_point(aes(x=SNM, y=predicted))
```
