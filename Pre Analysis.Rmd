...
mut <- rv %>% dplyr::select(CHROM, POS, TYPE, ploidy, HET, MAT, RDH)
...
```{r}
d <- d %>% left_join(rep[, c("chr", "pos", "rep.fitted.hap", "rep.fitted.dip", "rep.sign.hap", "rep.sign.dip")], by=c("chr", "pos"))
rm(rep)
...
```{r}
f.len <- read.delim("/outs/lengths.csv", sep=",")
f.len <- f.len %>% group_by(chr, pos) %>% summarize(length=mean(length))

head(f.len)

d <- d %>% left_join(f.len, by=c("chr", "pos"))
# d$length <- replace(d$length, is.na(d$length), 0)
head(d)
```

```{r}
f.len <- read.delim("/outs/distances.csv", sep=",")
f.len <- f.len %>% group_by(chr, pos) %>% summarize(distance=mean(distance))
...
```{r}
write.table(d, "/outs/the_greater_big_data_file_bigger.txt", row.names=FALSE)
```
