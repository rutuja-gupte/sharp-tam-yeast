...
print("loading data")
d <- read.delim("/outs/the_greater_big_data_file_length.txt", header=TRUE, sep=" ")
...
lm1 <- glm(formula = SNM ~ base.type + collision + transcripts + rep.fitted +
    length + protein + length:protein + base.type:protein + base.type:length +
    transcripts:rep.fitted + collision:rep.fitted + base.type:collision,
...
lm2 <- glm(SNM ~ (base.type + rep.fitted + protein +
             transcripts + collision + length)^2, 
           d, family="binomial")
print(summary(lm2))
...
lm1 <- glm(formula = SNM ~ base.type + collision + transcripts + rep.fitted +
    stops + length + protein + length:protein + collision:stops +
    base.type:protein + base.type:length + rep.fitted:protein +
    base.type:collision + transcripts:length + collision:rep.fitted,
    family = "binomial", data = d)
print(summary(lm1))
print(1 - (summary(lm1)$deviance/summary(lm1)$null.deviance))
...
lm2 <- glm(SNM ~ (base.type + rep.fitted + protein +
                    transcripts + collision + stops + length)^2, 
           d, family="binomial")
print(summary(lm2))
print(1 - (summary(lm2)$deviance/summary(lm2)$null.deviance))
